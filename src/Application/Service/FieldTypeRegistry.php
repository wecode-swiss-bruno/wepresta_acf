...
final class FieldTypeRegistry
{
    ...
    public function register(FieldTypeInterface $fieldType): self
    {
        ...
        if (isset($this->types[$type])) {
            throw new InvalidArgumentException(\sprintf('Field type "%s" already registered.', $type));
        }
    ...
    public function registerOrReplace(FieldTypeInterface $fieldType): self
    {
...
return $this->types[$type];
    ...
    public function getAllGroupedByCategory(): array
    {
        ...
        foreach ($this->types as $type => $fieldType) {
            $grouped[$fieldType->getCategory()][$type] = $fieldType;
        }
     ...
     */
    public function normalizeValue(string $type, mixed $value, array $config = []): mixed
    {
        return $this->getOrNull($type)?->normalizeValue($value, $config) ?? $value;
    }

    /**
     * @param array<string, mixed> $config
     */
    public function denormalizeValue(string $type, mixed $value, array $config = []): mixed
    {
        return $this->getOrNull($type)?->denormalizeValue($value, $config) ?? $value;
    }

    /**
     * @param array<string, mixed> $config @param array<string, mixed> $options
     */
    public function renderValue(string $type, mixed $value, array $config = [], array $options = []): string
    {
        return $this->getOrNull($type)?->renderValue($value, $config, $options) ?? htmlspecialchars((string) $value, ENT_QUOTES, 'UTF-8');
    }

    /**
     * @param array<string, mixed> $config
     */
    public function getIndexValue(string $type, mixed $value, array $config = []): ?string
    {
        return $this->getOrNull($type)?->getIndexValue($value, $config) ?? ($value === null ? null : substr((string) $value, 0, 255));
    }

    public function getFormType(string $type): ?string
    {
        return $this->getOrNull($type)?->getFormType();
    }

    /**
     * @param array<string, mixed> $config @param array<string, mixed> $validation @return array<string, mixed>
     */
    public function getFormOptions(string $type, array $config = [], array $validation = []): array
    {
        return $this->getOrNull($type)?->getFormOptions($config, $validation) ?? [];
    }

    /**
     * @param array<string, mixed> $config @param array<string, mixed> $validation @return array<string>
     */
    public function validate(string $type, mixed $value, array $config = [], array $validation = []): array
    {
