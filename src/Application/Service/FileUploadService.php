...
final class FileUploadService
{
    ...
    public function getPublicUrl(string $type, string $filename): string { return $this->moduleUrl . $type . '/' . $filename; }
    ...
    {
        if (!is_dir($dir) && !mkdir($dir, 0o755, true)) { throw new \RuntimeException('Failed to create upload directory: ' . $dir); }
        $htaccess = $dir . '.htaccess';
        if (!file_exists($htaccess)) {
            file_put_contents($htaccess, "# Deny direct PHP execution\n<FilesMatch \"\\.php$\">\nOrder allow,deny\nDeny from all\n</FilesMatch>\nOptions -ExecCGI -Indexes\nRemoveHandler .php .phtml .php3 .php4 .php5 .php7 .phps");
        }
        $indexPhp = $dir . 'index.php';
        if (!file_exists($indexPhp)) { file_put_contents($indexPhp, '<?php header("HTTP/1.0 403 Forbidden"); exit;'); }
    }
    ...
    {
        if (!filter_var($url, FILTER_VALIDATE_URL)) { throw new \InvalidArgumentException('Invalid URL format'); }
        $scheme = parse_url($url, PHP_URL_SCHEME);
        if (!in_array(strtolower($scheme ?? ''), ['http', 'https'], true)) { throw new \InvalidArgumentException('Only HTTP(S) URLs allowed'); }

        $tempFile = tempnam(sys_get_temp_dir(), 'wepresta_acf_');
        if ($tempFile === false) { throw new \RuntimeException('Failed to create temporary file'); }

        try {
            $ch = curl_init($url);
            $fp = fopen($tempFile, 'wb');
            if ($ch === false || $fp === false) { throw new \RuntimeException('Failed to initialize download'); }
            ...
            if ($result === false || $httpCode !== 200) { throw new \RuntimeException('Failed to download: ' . ($error ?: 'HTTP ' . $httpCode)); }
            $fileSize = filesize($tempFile);
            if ($fileSize === false || $fileSize === 0) { throw new \RuntimeException('Downloaded file is empty'); }

            $mimeType = $this->getMimeType($tempFile);
            if (!empty($allowedMimes) && !in_array($mimeType, $allowedMimes, true)) { throw new \InvalidArgumentException('File type not allowed: ' . $mimeType); }
            ...
            $destination = $dir . $filename;
            if (!copy($tempFile, $destination)) { throw new \RuntimeException('Failed to save downloaded file'); }
            chmod($destination, 0o644);
