...
*}
<div class="acf-entity-fields" id="acf-entity-fields" data-entity-type="{$acf_entity_type|escape:'htmlall':'UTF-8'}"
    data-entity-id="{$acf_entity_id|intval}" data-api-url="{$acf_api_base_url|escape:'htmlall':'UTF-8'}">
    ...
    <div class="acf-group card mb-3" data-group-id="{$group.id|intval}">
        ...
        <div class="card-body">
            ...
            {/if}
            <div class="{$fieldClasses}" {if isset($field.wrapper.id) && $field.wrapper.id}
                id="{$field.wrapper.id|escape:'html':'UTF-8'}" {/if}
                data-field-slug="{$field.slug|escape:'html':'UTF-8'}">
                <div class="{$labelColClass} text-md-left">
                    <label class="form-control-label{if $field.required} required{/if}">
                        {$field.title|escape:'html':'UTF-8'}
                        {if $field.required}<span class="text-danger">*</span>{/if}
                        {if $field.translatable}<span class="acf-translatable-badge"
                            title="{l s='Translatable field' d='Modules.Weprestaacf.Admin'}">🌐</span>{/if}
                    </label>
                ...
                <div class="{$inputColClass} acf-field-input">
                    ...
                    <div class="translations tabbable" id="acf_{$field.slug}" tabindex="1">
                        <ul class="translationsLocales nav nav-pills">
                            ...
                            <li class="nav-item">
                                <a href="#" data-locale="{$lang_input.iso_code|lower}"
                                    class="nav-link{if $lang_input.is_default} active{/if}" data-toggle="tab"
...
<style>
    ...
    .acf-field[data-field-presentation] {
        /* Custom field styling based on wrapper properties */
    }

    .acf-field .acf-field-input {
        /* Ensure input areas maintain proper spacing */
    }
</style>