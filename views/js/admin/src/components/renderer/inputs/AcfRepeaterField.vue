...
const repeaterConfig = computed(() => ({
  min: (props.field.config?.min as number) || 0,
  max: (props.field.config?.max as number) || 0,
  displayMode: (props.field.config?.displayMode as string) || 'table',
  buttonLabel: (props.field.config?.buttonLabel as string) || 'Add Row',
  rowTitle: (props.field.config?.rowTitle as string) || 'Row {#}',
  collapsed: (props.field.config?.collapsed as boolean) || false,
}))
...
function isTranslatable(field: AcfField): boolean {
  return !!field.value_translatable || !!field.translatable
}
